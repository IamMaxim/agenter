...
- Task 3.1 adds `AGENTER_RUNNER_MODE=codex` runner mode. It advertises the configured `AGENTER_WORKSPACE`, lazily launches a persistent `codex app-server --listen stdio://`, creates native Codex threads during Agenter session creation, starts read-only turns on the stored native thread id, normalizes known Codex JSON-RPC notifications/requests into `AppEvent`s, and forwards approval answers back to the provider request id.
- Native Codex thread ids are stored as `external_session_id` in the control-plane session registry and sent back on later browser messages. Full database-backed session registry recovery remains future work; the Postgres schema already has `agent_sessions.external_session_id`.
...
- Codex first-message lifecycle follow-up keeps runner-local knowledge of threads created or resumed by the current `codex app-server` process. The first turn for a freshly created native thread skips `thread/resume`; persisted or imported threads still resume before `turn/start`. If Codex rejects resume with `no rollout found for thread id ...`, the runner treats it as a pre-first-turn thread and starts the turn directly, while real missing-thread errors still fail visibly.
...
- Workspace provider refresh follow-up adds a browser-triggered `POST /api/workspaces/{workspace_id}/providers/{provider_id}/sessions/refresh` path. The control plane sends a runner `RefreshSessions` command, imports provider-discovered sessions in forced mode, rewrites caches only when provider history loaded successfully, and keeps old caches when a provider `thread/read` fails. Codex persisted `contextCompaction` items now import as visible provider events.
- Slash command follow-up adds generic command metadata and execution types shared across control plane, runner, and browser. The browser composer now shows an inline slash menu with descriptions, provider/danger badges, keyboard navigation, argument hints, and explicit confirmation for dangerous commands. The control plane exposes `GET/POST /api/sessions/{session_id}/slash-commands`, executes local commands such as settings/title/refresh/new, and dispatches provider commands through runner `ExecuteProviderCommand`. Codex currently advertises provider-heavy commands for compact, review, steer, fork, archive, unarchive, rollback, and unsandboxed shell execution; Qwen/ACP keeps an empty provider-native manifest until ACP commands are validated.
- Chat event visibility follow-up makes the control plane responsible for browser-submitted input echo and slash invocation visibility. Normal sends publish a first-party `UserMessageEvent` before runner dispatch, slash commands publish both a raw user echo and compact result provider event, Codex compaction remains a provider event, and browser error cards now retain structured code/message/provider detail for debugging. Providers still own native execution and native provider events.
- ACP runner support follow-up replaces the legacy Qwen-only production adapter with a shared ACP runtime and provider profiles for Qwen, Gemini, and OpenCode. The Qwen spike binary remains for direct protocol probing, while production `qwen`, `gemini`, `opencode`, and multi-provider `acp` runner modes use `crates/agenter-runner/src/agents/acp.rs`.
- Runner transport hardening follow-up replaces payload compaction with bidirectional runner WebSocket chunking. Large `RunnerClientMessage` and `RunnerServerMessage` payloads are split into bounded chunk frames and reassembled before normal protocol decoding, preserving full Codex provider payloads and discovered history while avoiding the default 16 MiB frame limit.
- Codex event-state follow-up normalizes observed lifecycle, token-usage, rate-limit, slash-result, and raw compaction payloads into stable app events. Plan deltas are now marked as append-only so browser replay reconstructs streamed implementation plans, and the browser shows explicit working/waiting/completed/failed state with a transient animated working row.
